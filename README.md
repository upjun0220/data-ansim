...
- **설계 기준:** `docs/1_팀공유_전체설계_V11.6_20260925.html`(수식·근거·Q&A)
...
- 테스트 113개 통과·3개 생략(미설치 선택 패키지). 2026-09-22 반입 데이터 2~10번 준비 완료(아래 "반입 준비 상태"). 이전 버전 문서(V9·V10 설계, 로드맵북 v10, V11.3 설계, 연구계획서 0917·0919)는 저장소에서 지웠으며 git 기록에 남아 있다.

> ⚠ `data/mock/` 은 전부 **가상** 데이터다. 로더·파이프라인 동작 검증용이며 결론에 쓰지 않는다.

## v11 — 현재 기준 (`docs/1_팀공유_전체설계_V11.6_20260925.html`)

**현재 설계 기준은 `docs/`의 V11.6 HTML(V11.3 설계 + 구현 + 현장 확인·심사 점검 반영)이다.** 이 절 아래의 V10 서술(구조표·폴백·설계 결정·mock 검증 결과)은 이력이며, 이 절과
다르면 이 절이 우선한다.
...
### v11.6 — CAN(티벨) 고유 데이터 활용 확대 (2026-09-25)

- **원정 충전**(`canloader.infer_home`·`away_charging`, CAN 식별번호가 개별 차량일 때만): 차량별 밤(21~06시) 가장 오래 머문
  200m 격자를 거주지로 추정(3밤 이상·절반 이상 — 새벽 주행이 거주지를 바꾸지 않게 '마지막 위치'가 아니라 '가장 오래 머문 곳'),
  거주지에서 1km 넘는 충전을 원정으로 보고 거주 법정동별로 집계한다(`s3_can_away_summary`·`s3_can_away_region`, 차량 3대 미만 동네 가림).
  차량 단위 결과는 저장·반출하지 않는다.
- **형평성 축 검증**(8-C 다음 단계, `away_access_check`): 원정 충전 세션 비율(주)·거주지 충전 없는 차량 비율(보조)과 2SFCA 접근성의
  순위 상관, 접근성 하위 20% 동네 대 나머지 비교(`s8c_can_away_check`) → `s9_story`의 “고유 데이터 발견(CAN)” 행.
- **시간 이동 여지**(`soc_flexibility`): KEPCO 피크 4시각에 시작한 충전의 시작 SOC 분포와 50% 이상 비율(`s3_can_flex_summary`·`_bands`).
- 설정: `can.home_hours`·`home_min_nights`·`home_min_share`·`away_km`·`flex_soc`·`peak_hours`. 현장 번들 `dist/day1/day1bundle4.py`.
...
LP는 방전(`p_discharge`)을 목표 초과분으로 등호 고정한 안전 하드 제약이며, 최적화 자유도는 비첨두 재충전 배분에만 있다. 후보군(`DEFAULT_CANDIDATES`)은 모두 방전 지속시간 약 2.85시간(0.35C)이라 장시간 피크에서 잔여 초과가 남을 수 있다. 법정동 배분에 쓰는 8.csv 면적 가중치는 산지·하천이 넓은 동네에서 MAUP(생태학적 오류) 왜곡 가능성이 있다.
...
**반입 준비 상태 (2026-09-22) — 데이터 2~10번 모두 준비됨.** 폴더는 셋이다: `dist/import`(코드 20개·설정·달력·README·5·9.csv) · `dist/import2`(2·3·4·6·7·8.csv·10.ttf) · `dist/import3`(갱신된 `fieldtemplate.txt`·`README.txt`·`checksums.txt`). 셋을 한 폴더로 합치고, 이름이 겹치는 파일은 import3 것을 쓴다(합계 약 12MB).

| 파일 | 출처·기준일 | 한계 |
|---|---|---|
| 2.csv | 국토교통부 전국 법정동 CSV(공공데이터포털 15063424) 2026-07-29 | — |
| 3.csv · 4.csv | 2.csv + GIS Developer 읍면동 경계 emd_20230729 → `tools/build_reference_files.py` | 서울 전부 매칭. 미매칭 7곳은 경기·충북·경북 |
| 6.csv | 한국환경공단 충전소 위치 및 운영정보(**2022-10-27**) 서울 33,792기를 카카오 로컬 API로 좌표 변환(`tools/geocode_stations.py`) | 4년 전 스냅샷. 6,585곳 중 6,494곳(충전기 99.2%) 변환, 좌표 4,488행. 이용자 제한 미반영 |
| 7.csv | 서울 열린데이터광장 OA-21236 월별 xlsx, 2023-01~2026-08 44개월(`tools/build_import2.py`) | 행정동 이름 미매칭 전기차 몫 약 1.6%(킬 20 경고선 10%). 2026-06 서울 전기차 117,055대 |
| 8.csv | 행정동 경계(vuski/admdongkor 2026-07-01) × 법정동 경계 면적 겹침, 427개 행정동·1,106행 | 가중치가 전기차 몫이 아니라 **면적 비율**. 행정동 2026-07 vs 법정동 2023-07 경계 |
| 10.ttf | NanumGothic(OFL) | 서버에 한글 폰트가 있으면 불필요 |

원본은 `data/ref/`(저장소 밖). 카카오 키는 환경변수 `KAKAO_REST_KEY`로만 썼고 저장소에 남기지 않았다.

**상호제공 정의서·홈페이지 카탈로그 대조(2026-09-22):** KEPCO_001·002는 대회 상호제공 목록에 있다(법정동 단위, 2023-01~2025-12, 002 가공일자 20250825). CAN은 일반 카탈로그 `TBE_001·002`(티벨 M·G-Type), KEP_007은 `KEP_007`, 지표 3 후보는 `KLH_001`(시군구), 전기차 등록 대체 후보는 `KTS_014`(2024-04~2026-03)다. 신청 범위에 들어가는지는 신청서에서 확인한다.
...
7. 6.csv는 2022-10 스냅샷이고 8.csv는 면적 가중치다. 센터의 KEP_007(충전소 설치현황)·KTS_014(등록)·행정동별 자료로 보강할 수 있는지 확인한다.
8. CAN(TBE_001·002)·KEP_007·KTS_014·KLH_001이 우리 신청 범위에 들어가는지 확인한다.
9. 9.csv(Open-Meteo)는 기상청 원자료가 아니다. 센터의 `KIS_010`(ASOS·AWS 정규격자)로 바꿀 수 있는지 확인한다.
...
| KEPCO 시각 | `조회기간`(YYYYMMDDHH) · 별도 시각 열 · 시간대별 가로 열 모두 지원(`layout=auto`). 가로 열은 `시간대별 사용량NN` 접두형과 `01:00`~`24:00`·`1시` 시각 이름형을 모두 인식 | 현장 KEPCO_001은 `조회기간`(YYYYMMDD) + `01:00`~`24:00` 가로 열로 확인(2026-09-23). 1~24 표기는 자동으로 0~23(구간 끝 표기). 서로 다른 열이 같은 시각으로 읽히면 합치지 않고 오류. 가로 형식은 청크를 시간대 열 수로 나눠 펼친 뒤 행 수를 long과 같게 맞춘다 |
