...
전기차 충전 활성화가 인근 상권 매출에 주는 효과를 **삼중차분(외지/거주자 × 대기소비/대조 업종) + 스태거드 이벤트 스터디**로 추정하고, 상권 유형별 CATE 를 전력 부하 집중도와 결합해 4사분면 처방을 만든다.
...
| 묶음 | 내용 | 만드는 법 |
|---|---|---|
| `dist/ev-ripplemap_반입_<커밋>.zip` | 실행 코드 13 · 설정 템플릿 2 · README · requirements · 반입목록(SHA-256) | 커밋 후 zip 생성 |
...
| 001/002 | 합산 코드 없음. `kepco.source` 로 하나만 선택 | 킬 크라이테리아 7번은 포함관계 정황만 판정 |
...
Causal Forest 경로는 이 개발 환경에 econml 이 없어 **실행 검증하지 못했다**(폴백 2×2 만 검증). 현장에 econml 이 있으면 `s7_cate_validation` 에서 비교 결과를 확인할 것.
