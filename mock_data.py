...
심어 둔 정답(테스트가 확인한다):
  - 법정동 30곳(서울 가상 3개 구). 처치 18곳(2025-03~07 스태거드) · never-treated 10 · 창 이전 활성화 1 · 창 이후 1
...
SIGUNGU = [("710", "가람구"), ("720", "나래구"), ("730", "다솜구")]
EMD_NAMES = {
    "710": ["가람1동", "가람2동", "중앙동", "새솔동", "한울동", "별빛동", "누리동", "꽃내동", "솔밭동", "미리내동"],
    "720": ["나래1동", "나래2동", "중앙동", "은빛동", "푸른동", "햇살동", "초록동", "바람동", "구름동", "하늘동"],
    "730": ["다솜1동", "다솜2동", "노을동", "샘물동", "들꽃동", "산마루동", "달빛동", "새터동", "물빛동", "보람동"],
}
...
def write_master(reg, path):
    ...
    lines.append(f"{SIDO_CODE}730199" + "00\t" + f"{SIDO_FULL} 다솜구 옛터동\t폐지")
...
def write_kepco(reg, rng, path001, path002):
    ...
    hours = np.arange(24)
    extra = pd.DataFrame([{"sigungu": "다솜구", "kepco_emd": "행복동", "kwh_level": 30.0, "peakiness": 0.5,
